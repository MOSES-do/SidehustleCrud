...
$name = $roll = $reg = $uname = $email = $agree = $website =  '';
    ...
    //Unique username??? To ensure usernames and emails are not repeated in database more than the allowed limit
    $user_check_query = "SELECT * FROM proj3 WHERE uname = '$uname' || email = '$email' LIMIT 1";
    ...
    if($user['Uname'] == $uname){
        $error_msg['uname'] = "Username already exists";
    }
    if($user['Email'] == $email){
        $error_msg['email'] = "This email has a registered username";
        }
    }   
